...
const verificadorPontos = (array1, array2) => {
      if (array1 === array2) {
        return 1;
      } if (array2 === 'N.A') {
        return 0;
...
const corretorAutomatico = (solucoes , estudante, funcao) => {
  let verificador = 0;
  for (let i = 0; i < solucoes.length; i +=1) {
    const soma = funcao(solucoes[i], estudante[i]);
    verificador += soma;
  }
  return verificador;
}
console.log(corretorAutomatico(RIGHT_ANSWERS, STUDENT_ANSWERS, verificadorPontos));

// Exercicio extraido gabarito